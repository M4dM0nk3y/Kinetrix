...
static void pico_expr(CodeGen *gen, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_ANALOG_READ:
            pico_emit(gen, "_safe_adc("); pico_expr(gen, node->data.gpio.pin);
            pico_emit(gen, ")");
            break;
        case NODE_GPIO_READ:
        ...
        case NODE_ARRAY_ACCESS:
            ...
            break;
        case NODE_CAST:
            pico_emit(gen, "%s(", strcmp(type_to_ctype(node->data.cast_op.target_type), "float") == 0 ? "float" : "int");
        ...
        case NODE_MATH_FUNC: {
            ...
            if(node->data.math_func.arg2) { pico_emit(gen, ", "); pico_expr(gen, node->data.math_func.arg2); }
            pico_emit(gen, ")");
            break;
        }
    ...
    switch (node->type) {
        ...
        case NODE_IF:
            ...
            if (node->data.if_stmt.else_block) {
                ...
                if (else_b->type == NODE_BLOCK && else_b->data.block.statement_count == 1 &&
                    else_b->data.block.statements[0]->type == NODE_IF) {
                    ASTNode *elif = else_b->data.block.statements[0];
                    pico_indent(gen); pico_emit(gen, "elif ");
        ...
        case NODE_FOR: {
            ...
            pico_indent(gen);
            pico_emit(gen, "for %s in range(_start_%d, _end_%d + (1 if _step_%d > 0 else -1), _step_%d):\n", 
        ...
        case NODE_CONTINUE: pico_emit_line(gen, "continue\n"); break;
        ...
        case NODE_STRUCT_INSTANCE:
            pico_indent(gen); pico_emit(gen, "%s = {}\n", node->data.struct_instance.var_name);
            break;
        case NODE_INTERRUPT_PIN:
            pico_indent(gen);
            pico_emit(gen, "Pin(%d, Pin.IN).irq(trigger=Pin.IRQ_", node->data.interrupt_pin.pin_number);
            if(node->data.interrupt_pin.mode == INT_MODE_RISING) pico_emit(gen, "RISING");
            else if(node->data.interrupt_pin.mode == INT_MODE_FALLING) pico_emit(gen, "FALLING");
            else pico_emit(gen, "RISING | Pin.IRQ_FALLING");
            pico_emit(gen, ", handler=lambda p: _isr_pin%d())\n", node->data.interrupt_pin.pin_number);
            break;
        case NODE_INTERRUPT_TIMER:
            pico_indent(gen);
            pico_emit(gen, "machine.Timer().init(period=%d, mode=machine.Timer.PERIODIC, callback=lambda t: _isr_timer())\n", 
                node->data.interrupt_timer.interval);
            break;
        case NODE_DISABLE_INTERRUPTS: pico_emit_line(gen, "_irq_state = machine.disable_irq()"); break;
        case NODE_ENABLE_INTERRUPTS: pico_emit_line(gen, "machine.enable_irq(_irq_state)"); break;
        case NODE_I2C_OPEN: pico_emit_line(gen, "_i2c = I2C(0, scl=Pin(5), sda=Pin(4), freq=400000)"); break;
        case NODE_I2C_DEVICE_READ_ARRAY:
            pico_indent(gen);
        ...
        case NODE_I2C_DEVICE_WRITE:
            ...
            pico_emit(gen, ", ");
            pico_expr(gen, node->data.i2c_device_write.value); // register? or just raw bytes? wait, Kinetrix uses value.
            pico_emit(gen, ", bytes([");
            pico_expr(gen, node->data.i2c_device_write.value);
            pico_emit(gen, "]))\n");
            break;
        case NODE_SERIAL_OPEN: pico_emit_line(gen, "_uart = UART(0, baudrate=%d, tx=Pin(0), rx=Pin(1))", node->data.serial_open.baud_rate); break;
        case NODE_SERIAL_SEND:
            pico_indent(gen); pico_emit(gen, "if '_uart' in globals(): _uart.write(str(");
            pico_expr(gen, node->data.serial_send.value); pico_emit(gen, "))\n"); break;
        case NODE_SPI_OPEN: pico_emit_line(gen, "_spi = machine.SPI(0, baudrate=%d)", node->data.spi_open.frequency > 0 ? node->data.spi_open.frequency : 1000000); break;
        case NODE_DEVICE_DEF: pico_indent(gen); pico_emit(gen, "# Device %s configured\n", node->data.device_def.device_name); break;
        case NODE_DEVICE_WRITE: pico_indent(gen); pico_emit(gen, "# Write %s: ", node->data.device_write.device_name); pico_expr(gen, node->data.device_write.value); pico_emit(gen, "\n"); break;
        case NODE_TASK_DEF: break;
        case NODE_TASK_START:
            pico_emit_line(gen, "import _thread");
            pico_emit_line(gen, "try: _thread.start_new_thread(_task_func_%s, ())", node->data.task_start.task_name);
            pico_emit_line(gen, "except Exception: pass");
            break;
        case NODE_TRY:
            pico_emit_line(gen, "try:"); gen->indent_level++; pico_stmt(gen, node->data.try_stmt.try_block); gen->indent_level--;
            if (node->data.try_stmt.error_block) {
                pico_emit_line(gen, "except Exception:"); gen->indent_level++; pico_stmt(gen, node->data.try_stmt.error_block); gen->indent_level--;
            }
            break;
        case NODE_ASSERT:
            pico_indent(gen); pico_emit(gen, "if not ("); pico_expr(gen, node->data.assert_stmt.condition); pico_emit(gen, "):\n");
            gen->indent_level++;
            if (node->data.assert_stmt.action) pico_stmt(gen, node->data.assert_stmt.action);
            else pico_emit_line(gen, "raise Exception('Assertion Failed')");
            gen->indent_level--; break;
        case NODE_RADIO_SEND:
            ...
            pico_emit(gen, ")\n");
            break;
        case NODE_WATCHDOG_ENABLE: pico_emit_line(gen, "_wdt = machine.WDT(timeout=%d)", node->data.watchdog_enable.timeout_ms); break;
        case NODE_WATCHDOG_FEED: pico_emit_line(gen, "if '_wdt' in globals(): _wdt.feed()"); break;
        case NODE_OTA_ENABLE:
            pico_emit_line(gen, "# OTA: Enable WebREPL for wireless code push (Pico W)");
            ...
            pico_emit_line(gen, "        print('OTA ready via WebREPL at', wlan.ifconfig()[0])");
            pico_emit_line(gen, "except: print('OTA: WiFi not available on this Pico variant')");
            break;
        case NODE_TONE:
            pico_indent(gen); pico_emit(gen, "_pwm = PWM(Pin("); pico_expr(gen, node->data.gpio.pin);
            pico_emit(gen, ")); _pwm.freq(int("); pico_expr(gen, node->data.gpio.value);
            pico_emit(gen, ")); _pwm.duty_u16(32768)\n"); break;
        case NODE_NOTONE:
            pico_indent(gen); pico_emit(gen, "PWM(Pin("); pico_expr(gen, node->data.gpio.pin); pico_emit(gen, ")).deinit()\n"); break;
...
void codegen_generate_pico(CodeGen *gen, ASTNode *program) {
    ...
    pico_emit_line(gen, "    pass # Placeholder for Pico NRF24 or ESP-NOW transmission\n");
    ...
    if (block && block->type == NODE_BLOCK) {
        ...
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ...
            if (stmt->type == NODE_INTERRUPT_PIN) {
                pico_emit_line(gen, "def _isr_pin%d():", stmt->data.interrupt_pin.pin_number);
                gen->indent_level++;

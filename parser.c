...
void lexer_next_token(Lexer *lexer) {
    ...
    if (lexer->current_char == '"') {
        ...
        while (lexer->current_char != '"' && lexer->current_char != EOF) {
            if (i >= lexer->current_token.capacity - 2) {
                ...
                lexer->current_token.value = realloc(lexer->current_token.value, lexer->current_token.capacity);
            }
    ...
    if (lexer->current_char == '0') {
        ...
        if (lexer->current_char == 'x' || lexer->current_char == 'X') {
            ...
            snprintf(lexer->current_token.value, lexer->current_token.capacity, "%ld", val);
            return;
    ...
    if (isalpha(lexer->current_char) || lexer->current_char == '_') {
        ...
        if      (!strcmp(v,"program"))   lexer->current_token.type = TOK_PROGRAM;
        else if (!strcmp(v,"extern"))    lexer->current_token.type = TOK_EXTERN;
        else if (!strcmp(v,"def"))       lexer->current_token.type = TOK_DEF;
        else if (!strcmp(v,"repeat"))    lexer->current_token.type = TOK_REPEAT;
        else if (!strcmp(v,"loop"))      lexer->current_token.type = TOK_LOOP;
        else if (!strcmp(v,"turn"))      lexer->current_token.type = TOK_TURN;
        else if (!strcmp(v,"on"))        lexer->current_token.type = TOK_ON;
        else if (!strcmp(v,"off"))       lexer->current_token.type = TOK_OFF;
        else if (!strcmp(v,"wait"))      lexer->current_token.type = TOK_WAIT;
        else if (!strcmp(v,"wait_us"))   lexer->current_token.type = TOK_WAIT_US;
        else if (!strcmp(v,"forever"))   lexer->current_token.type = TOK_FOREVER;
        else if (!strcmp(v,"if"))        lexer->current_token.type = TOK_IF;
        else if (!strcmp(v,"else"))      lexer->current_token.type = TOK_ELSE;
        else if (!strcmp(v,"make"))      lexer->current_token.type = TOK_MAKE;
        else if (!strcmp(v,"var"))       lexer->current_token.type = TOK_VAR;
        else if (!strcmp(v,"set"))       lexer->current_token.type = TOK_SET;
        else if (!strcmp(v,"change"))    lexer->current_token.type = TOK_CHANGE;
        else if (!strcmp(v,"to"))        lexer->current_token.type = TOK_TO;
        else if (!strcmp(v,"by"))        lexer->current_token.type = TOK_BY;
        else if (!strcmp(v,"read"))      lexer->current_token.type = TOK_READ;
        else if (!strcmp(v,"write"))     lexer->current_token.type = TOK_WRITE_KW;
        else if (!strcmp(v,"analog"))    lexer->current_token.type = TOK_ANALOG;
        else if (!strcmp(v,"pulse"))     lexer->current_token.type = TOK_PULSE;
        else if (!strcmp(v,"serial"))    lexer->current_token.type = TOK_SERIAL;
        else if (!strcmp(v,"servo"))     lexer->current_token.type = TOK_SERVO;
        else if (!strcmp(v,"print"))     lexer->current_token.type = TOK_PRINT;
        else if (!strcmp(v,"println"))   lexer->current_token.type = TOK_PRINTLN;
        else if (!strcmp(v,"tone"))      lexer->current_token.type = TOK_TONE;
        else if (!strcmp(v,"notone"))    lexer->current_token.type = TOK_NOTONE;
        else if (!strcmp(v,"return"))    lexer->current_token.type = TOK_RETURN;
        else if (!strcmp(v,"while"))     lexer->current_token.type = TOK_WHILE;
        else if (!strcmp(v,"for"))       lexer->current_token.type = TOK_FOR;
        else if (!strcmp(v,"from"))      lexer->current_token.type = TOK_FROM;
        else if (!strcmp(v,"break"))     lexer->current_token.type = TOK_BREAK;
        else if (!strcmp(v,"array"))     lexer->current_token.type = TOK_ARRAY;
        else if (!strcmp(v,"buffer"))    lexer->current_token.type = TOK_BUFFER;
        else if (!strcmp(v,"push"))      lexer->current_token.type = TOK_PUSH;
        /* size, index, count, into, value, start, stop, at, as — context-sensitive, kept as TOK_ID */
        else if (!strcmp(v,"of"))        lexer->current_token.type = TOK_OF;
        else if (!strcmp(v,"i2c"))       lexer->current_token.type = TOK_I2C;
        else if (!strcmp(v,"spi"))       lexer->current_token.type = TOK_SPI;
        else if (!strcmp(v,"begin"))     lexer->current_token.type = TOK_BEGIN;
        else if (!strcmp(v,"start"))     lexer->current_token.type = TOK_START;
        else if (!strcmp(v,"send"))      lexer->current_token.type = TOK_SEND;
        else if (!strcmp(v,"stop"))      lexer->current_token.type = TOK_STOP;
        else if (!strcmp(v,"open"))      lexer->current_token.type = TOK_OPEN;
        else if (!strcmp(v,"baud"))      lexer->current_token.type = TOK_BAUD;
        else if (!strcmp(v,"hz"))        lexer->current_token.type = TOK_HZ;
        else if (!strcmp(v,"transfer"))  lexer->current_token.type = TOK_TRANSFER;
        else if (!strcmp(v,"receive"))   lexer->current_token.type = TOK_RECEIVE;
        else if (!strcmp(v,"device"))    lexer->current_token.type = TOK_DEVICE;
        else if (!strcmp(v,"register"))  lexer->current_token.type = TOK_REGISTER;
        else if (!strcmp(v,"pin"))       lexer->current_token.type = TOK_PIN;
        else if (!strcmp(v,"is"))        lexer->current_token.type = TOK_IS;
        else if (!strcmp(v,"high"))      lexer->current_token.type = TOK_HIGH;
        else if (!strcmp(v,"low"))       lexer->current_token.type = TOK_LOW;
        else if (!strcmp(v,"sin"))       lexer->current_token.type = TOK_SIN;
        else if (!strcmp(v,"cos"))       lexer->current_token.type = TOK_COS;
        else if (!strcmp(v,"tan"))       lexer->current_token.type = TOK_TAN;
        else if (!strcmp(v,"and"))       lexer->current_token.type = TOK_AND;
        else if (!strcmp(v,"or"))        lexer->current_token.type = TOK_OR;
        else if (!strcmp(v,"sqrt"))      lexer->current_token.type = TOK_SQRT;
        else if (!strcmp(v,"asin"))      lexer->current_token.type = TOK_ASIN;
        else if (!strcmp(v,"acos"))      lexer->current_token.type = TOK_ACOS;
        else if (!strcmp(v,"atan"))      lexer->current_token.type = TOK_ATAN;
        else if (!strcmp(v,"atan2"))     lexer->current_token.type = TOK_ATAN2;
        else if (!strcmp(v,"include"))   lexer->current_token.type = TOK_INCLUDE;
        else if (!strcmp(v,"import"))    lexer->current_token.type = TOK_INCLUDE;
        /* V3.0 explicit types */
        else if (!strcmp(v,"int"))       lexer->current_token.type = TOK_INT_KW;
        else if (!strcmp(v,"float"))     lexer->current_token.type = TOK_FLOAT_KW;
        else if (!strcmp(v,"bool"))      lexer->current_token.type = TOK_BOOL_KW;
        else if (!strcmp(v,"byte"))      lexer->current_token.type = TOK_BYTE_KW;
        else if (!strcmp(v,"cast"))      lexer->current_token.type = TOK_CAST;
        else if (!strcmp(v,"true"))      lexer->current_token.type = TOK_TRUE;
        else if (!strcmp(v,"false"))     lexer->current_token.type = TOK_FALSE;
        /* V3.0 interrupt */
        else if (!strcmp(v,"rising"))    lexer->current_token.type = TOK_RISING;
        else if (!strcmp(v,"falling"))   lexer->current_token.type = TOK_FALLING;
        else if (!strcmp(v,"changing"))  lexer->current_token.type = TOK_CHANGING;
        else if (!strcmp(v,"timer"))     lexer->current_token.type = TOK_TIMER;
        else if (!strcmp(v,"every"))     lexer->current_token.type = TOK_EVERY;
        else if (!strcmp(v,"us"))        lexer->current_token.type = TOK_US_TOK;
        else if (!strcmp(v,"ms"))        lexer->current_token.type = TOK_MS_TOK;
        /* V3.0 error handling */
        else if (!strcmp(v,"try"))       lexer->current_token.type = TOK_TRY;
        else if (!strcmp(v,"enable"))    lexer->current_token.type = TOK_ENABLE;
        else if (!strcmp(v,"watchdog"))  lexer->current_token.type = TOK_WATCHDOG;
        else if (!strcmp(v,"feed"))      lexer->current_token.type = TOK_FEED;
        else if (!strcmp(v,"assert"))    lexer->current_token.type = TOK_ASSERT;
        else if (!strcmp(v,"timeout"))   lexer->current_token.type = TOK_TIMEOUT;
        else if (!strcmp(v,"disable"))   lexer->current_token.type = TOK_DISABLE;
        else if (!strcmp(v,"interrupts"))lexer->current_token.type = TOK_INTERRUPTS;
        /* V3.0 structs/define */
        else if (!strcmp(v,"define"))    lexer->current_token.type = TOK_DEFINE;
        else if (!strcmp(v,"type"))      lexer->current_token.type = TOK_TYPE_KW;
        /* V3.0 concurrency */
        else if (!strcmp(v,"task"))      lexer->current_token.type = TOK_TASK;
        else if (!strcmp(v,"shared"))    lexer->current_token.type = TOK_SHARED;
        /* V3.0 explicit types */
        else if (!strcmp(v,"continue"))  lexer->current_token.type = TOK_CONTINUE;
        else if (!strcmp(v,"radio_send_peer")) lexer->current_token.type = TOK_RADIO_SEND;
        else if (!strcmp(v,"radio_available")) lexer->current_token.type = TOK_RADIO_AVAILABLE;
        else if (!strcmp(v,"radio_read"))      lexer->current_token.type = TOK_RADIO_READ;
        /* V3.0 handled as TOK_ID in parser */
        else if (!strcmp(v,"not"))       lexer->current_token.type = TOK_NOT;
        else if (!strcmp(v,"const"))     lexer->current_token.type = TOK_ID;
        else if (!strcmp(v,"map"))       lexer->current_token.type = TOK_ID;
        else if (!strcmp(v,"constrain")) lexer->current_token.type = TOK_ID;
        else if (!strcmp(v,"abs"))       lexer->current_token.type = TOK_ID;
...
static const char* token_type_to_string(TokenType type) {
    switch (type) {
        ...
        case TOK_ANALOG: return "analog";
        case TOK_PULSE: return "pulse";
        case TOK_SERVO: return "servo";
        case TOK_PRINT: return "print";
        case TOK_PRINTLN: return "println";
        case TOK_DEF: return "def";
        case TOK_REPEAT: return "repeat";
        case TOK_TONE: return "tone";
        case TOK_NOTONE: return "notone";
        case TOK_FREQ: return "freq";
        ...
        case TOK_SERIAL: return "serial";
        case TOK_WAIT_US: return "wait_us";
        case TOK_I2C: return "i2c";
        ...
        case TOK_HIGH: return "HIGH";
        case TOK_LOW: return "LOW";
        ...
        case TOK_BYTE_KW: return "byte";
        case TOK_CAST: return "cast";
        case TOK_RISING: return "rising";
        case TOK_FALLING: return "falling";
        case TOK_CHANGING: return "changing";
        case TOK_TIMER: return "timer";
        case TOK_EVERY: return "every";
        case TOK_US_TOK: return "us";
        case TOK_MS_TOK: return "ms";
        case TOK_OPEN: return "open";
        case TOK_BAUD: return "baud";
        case TOK_HZ: return "hz";
        case TOK_SPI: return "spi";
        case TOK_TRANSFER: return "transfer";
        case TOK_RECEIVE: return "receive";
        case TOK_COUNT: return "count";
        case TOK_INTO: return "into";
        case TOK_WRITE_KW: return "write";
        case TOK_DEVICE: return "device";
        case TOK_REGISTER: return "register";
        case TOK_VALUE_KW: return "value";
        case TOK_AT: return "at";
        case TOK_DEFINE: return "define";
        case TOK_TYPE_KW: return "type";
        case TOK_AS: return "as";
        case TOK_TRY: return "try";
        case TOK_ON_ERROR: return "on_error";
        case TOK_ENABLE: return "enable";
        case TOK_WATCHDOG: return "watchdog";
        case TOK_FEED: return "feed";
        case TOK_ASSERT: return "assert";
        case TOK_TIMEOUT: return "timeout";
        case TOK_DISABLE: return "disable";
        case TOK_INTERRUPTS: return "interrupts";
        case TOK_TASK: return "task";
        case TOK_SHARED: return "shared";
        case TOK_BUFFER: return "buffer";
        case TOK_PUSH: return "push";
        case TOK_RADIO_SEND: return "radio_send_peer";
        case TOK_RADIO_AVAILABLE: return "radio_available";
        case TOK_RADIO_READ: return "radio_read";
...
int parser_expect(Parser *parser, TokenType type) {
    ...
    snprintf(msg, sizeof(msg), "Expected '%s', got '%s'%s%s", 
             ...
             parser->lexer->current_token.type == TOK_ID ? parser->lexer->current_token.value : "");
...
static ASTNode* parse_primary(Parser *parser) {
    ...
    if (parser_match(parser, TOK_READ)) {
        ...
        /* read i2c device <addr> register <reg> [count <n> into <array>] (V3.0 high-level) */
        if (parser_match(parser, TOK_I2C)) {
            lexer_next_token(parser->lexer);
            if (parser_match(parser, TOK_DEVICE)) { lexer_next_token(parser->lexer); }
            ASTNode *addr = parse_expression(parser);
            ASTNode *reg = NULL;
            if (parser_match(parser, TOK_REGISTER)) { lexer_next_token(parser->lexer); reg = parse_expression(parser); }
            ...
            if (parser_match_id(parser, "count")) {
                ...
                if (!parser_match_id(parser, "into")) {
                    error_report(parser->errors, ERROR_SYNTAX, parser->lexer->current_token.line, parser->lexer->current_token.column, "Expected 'into' after count expression");
                } else { lexer_next_token(parser->lexer); }
            ...
            if (reg) return ast_i2c_device_read(addr, reg);
        ...
        if (parser_match(parser, TOK_ID)) {
            ...
            if (sym && sym->kind == SYMBOL_DEVICE) proto = sym->protocol;
    ...
    if (parser_match(parser, TOK_TRANSFER)) {
        lexer_next_token(parser->lexer);
        if (parser_match(parser, TOK_SPI)) lexer_next_token(parser->lexer);
    ...
    if (parser_match(parser, TOK_CAST)) {
        lexer_next_token(parser->lexer);
        Type *t = type_float();
        if (parser_match(parser, TOK_INT_KW))        { t = type_int();   lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_FLOAT_KW)) { t = type_float(); lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_BOOL_KW))  { t = type_bool();  lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_BYTE_KW))  { t = type_byte();  lexer_next_token(parser->lexer); }
        ASTNode *operand = parse_primary(parser);
...
static ASTNode* parse_arithmetic(Parser *parser) {
    ...
    while (parser_match(parser, TOK_PLUS) || parser_match(parser, TOK_MINUS) ||
        ...
        else if (parser_match(parser, TOK_STAR)) op = OP_MUL;
        else if (parser_match(parser, TOK_SLASH)) op = OP_DIV;
        else if (parser_match(parser, TOK_MOD)) op = OP_MOD;
...
static ASTNode* parse_comparison(Parser *parser) {
    ...
    while (parser_match(parser, TOK_EQ) || parser_match(parser, TOK_NEQ) ||
        ...
        Operator op = OP_EQ;
        if      (parser_match(parser, TOK_NEQ)) op = OP_NEQ;
        else if (parser_match(parser, TOK_LTE)) op = OP_LTE;
        else if (parser_match(parser, TOK_GTE)) op = OP_GTE;
...
static ASTNode* parse_statement(Parser *parser) {
    ...
    if (parser_match(parser, TOK_MAKE)) {
        ...
        if (parser_match(parser, TOK_ARRAY)) {
            ...
            if (parser_match(parser, TOK_INT_KW))        { elem_type = type_int();   lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_FLOAT_KW)) { elem_type = type_float(); lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_BOOL_KW))  { elem_type = type_bool();  lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_BYTE_KW))  { elem_type = type_byte();  lexer_next_token(parser->lexer); }
        ...
        if (parser_match(parser, TOK_VAR)) {
            ...
            parser_expect(parser, TOK_ID);
            ASTNode *init = NULL;
            if (parser_match(parser, TOK_ASSIGN)) { lexer_next_token(parser->lexer); init = parse_expression(parser); }
            symbol_table_add(parser->symbols, name_tok.value, SYMBOL_VARIABLE, type_inferred(), name_tok.line);
            return ast_var_decl(name_tok.value, NULL, init);
        }

        /* make int|float|bool|byte <name> = <expr>   (V3.0 explicit type) */
        Type *decl_type = NULL;
        if (parser_match(parser, TOK_INT_KW))        { decl_type = type_int();   lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_FLOAT_KW)) { decl_type = type_float(); lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_BOOL_KW))  { decl_type = type_bool();  lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_BYTE_KW))  { decl_type = type_byte();  lexer_next_token(parser->lexer); }

        if (decl_type != NULL) {
            /* might be: make float name = expr   OR   make float ratio = cast float speed/255 */
            Token name_tok = parser->lexer->current_token;
            name_tok.value = strdup(name_tok.value);
            parser_expect(parser, TOK_ID);
            ASTNode *init = NULL;
            if (parser_match(parser, TOK_ASSIGN)) { lexer_next_token(parser->lexer); init = parse_expression(parser); }
            symbol_table_add(parser->symbols, name_tok.value, SYMBOL_VARIABLE, decl_type, name_tok.line);
        ...
        error_report(parser->errors, ERROR_SYNTAX,
            parser->lexer->current_token.line, parser->lexer->current_token.column,
            "Expected var/int/float/bool/byte/array/buffer or identifier after 'make'");
        return NULL;
    }
    ...
    // Change statement: change x by 5  /  change x.field by 5  /  change x[i] by 5
    if (parser_match(parser, TOK_CHANGE)) {
        ...
        /* Clone target manually for the right side of OP_ADD to prevent double-free in AST destruction */
    ...
    if (parser_match(parser, TOK_WAIT)) {
        ...
        if (parser_match(parser, TOK_MS_TOK)) {
            ...
            if (strcmp(unit, "seconds") == 0 || strcmp(unit, "s") == 0 || strcmp(unit, "sec") == 0 || strcmp(unit, "second") == 0) {
                duration = ast_binary_op(OP_MUL, duration, ast_number(1000));
                lexer_next_token(parser->lexer);
            } else if (strcmp(unit, "milliseconds") == 0 || strcmp(unit, "ms") == 0 || strcasecmp(unit, "millisecond") == 0) {
                lexer_next_token(parser->lexer);
            } else if (strcmp(unit, "microseconds") == 0 || strcmp(unit, "us") == 0 || strcasecmp(unit, "microsecond") == 0) {
                lexer_next_token(parser->lexer);
                ASTNode **args = malloc(sizeof(ASTNode*));
                args[0] = duration;
                return ast_call("delayMicroseconds", args, 1);
    ...
    // Print / Println
    if (parser->lexer->current_token.type == TOK_PRINT || parser->lexer->current_token.type == TOK_PRINTLN) {
    ...
    if (parser_match(parser, TOK_EXTERN)) {
        ...
        while (!parser_match(parser, TOK_RPAREN) && !parser_match(parser, TOK_EOF)) {
            ...
            if (parser_match(parser, TOK_COLON)) {
                ...
                } else if (parser_match(parser, TOK_INT_KW))   { ptype = type_int();   lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_FLOAT_KW))   { ptype = type_float(); lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_BOOL_KW))    { ptype = type_bool();  lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_BYTE_KW))    { ptype = type_byte();  lexer_next_token(parser->lexer); }
            }
        ...
        if (parser_match(parser, TOK_ARROW)) {
            ...
            } else if (parser_match(parser, TOK_INT_KW))   { ret_type = type_int();   lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_FLOAT_KW))   { ret_type = type_float(); lexer_next_token(parser->lexer); }
        }
        
        return ast_extern_function_def(name_tok.value, param_names, param_types, param_count, ret_type, lang_tok.value);
    }
    ...
    if (parser_match(parser, TOK_DEF)) {
        ...
        while (!parser_match(parser, TOK_RPAREN) && !parser_match(parser, TOK_EOF)) {
            ...
            Type *ptype = type_float(); /* default */
            if (parser_match(parser, TOK_INT_KW))        { ptype = type_int();   lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_FLOAT_KW)) { ptype = type_float(); lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_BOOL_KW))  { ptype = type_bool();  lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_BYTE_KW))  { ptype = type_byte();  lexer_next_token(parser->lexer); }
    ...
    if (parser_match(parser, TOK_LOOP)) {
        ...
        if (parser_match(parser, TOK_FOREVER)) {
            ...
            ASTNode *count = parse_expression(parser);
            if (parser_match(parser, TOK_ID) && strcmp(parser->lexer->current_token.value, "times") == 0) {
                lexer_next_token(parser->lexer);
            }
            parser_expect(parser, TOK_LBRACE);
            parser->in_loop++;
    ...
    if (parser_match(parser, TOK_FOR)) {
        ...
        symbol_table_enter_scope(parser->symbols);
        symbol_table_add(parser->symbols, var_tok.value, SYMBOL_VARIABLE, type_int(), var_tok.line);
        
        // Suppress unused warning since iterating a loop counts as semantic utilization
        symbol_table_lookup_current_scope(parser->symbols, var_tok.value)->is_used = 1;
    ...
    if (parser_match(parser, TOK_ON)) {
        ...
        if (parser_match(parser, TOK_PIN)) {
            ...
            if (parser_match(parser, TOK_RISING))        { mode = INT_MODE_RISING;   lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_FALLING))  { mode = INT_MODE_FALLING;  lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_CHANGING)) { mode = INT_MODE_CHANGING; lexer_next_token(parser->lexer); }
            parser_expect(parser, TOK_LBRACE);
            ASTNode *body = parse_block(parser);
            parser_expect(parser, TOK_RBRACE);
        ...
        if (parser_match(parser, TOK_TIMER)) {
            lexer_next_token(parser->lexer);
            /* expect 'every' */
            if (parser_match(parser, TOK_EVERY)) lexer_next_token(parser->lexer);
            ...
            lexer_next_token(parser->lexer);
            int is_us = 1;
            if (parser_match(parser, TOK_MS_TOK)) { is_us = 0; lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_US_TOK)) { is_us = 1; lexer_next_token(parser->lexer); }
            parser_expect(parser, TOK_LBRACE);
            ASTNode *body = parse_block(parser);
            parser_expect(parser, TOK_RBRACE);
    ...
    if (parser_match(parser, TOK_OPEN)) {
        ...
        if (parser_match(parser, TOK_SERIAL)) {
            lexer_next_token(parser->lexer);
            if (parser_match_id(parser, "at")) lexer_next_token(parser->lexer);
            Token baud_tok = parser->lexer->current_token;
            baud_tok.value = strdup(baud_tok.value);
            int baud = (int)atof(baud_tok.value);
            lexer_next_token(parser->lexer);
            if (parser_match(parser, TOK_BAUD)) lexer_next_token(parser->lexer);
        ...
        if (parser_match(parser, TOK_SPI)) {
            ...
            lexer_next_token(parser->lexer);
            if (parser_match(parser, TOK_HZ)) lexer_next_token(parser->lexer);
    ...
    if (parser_match(parser, TOK_WRITE_KW)) {
        ...
        if (parser_match(parser, TOK_I2C)) {
            ...
            if (parser_match(parser, TOK_DEVICE)) lexer_next_token(parser->lexer);
            ASTNode *addr = parse_expression(parser);
            if (parser_match_id(parser, "value")) lexer_next_token(parser->lexer);
            ASTNode *val = parse_expression(parser);
    ...
    if (parser_match(parser, TOK_READ)) {
        // We will push back if not I2C array read, or we can just parse it as an expression statement.
        // Since `read` is usually an expression, if it sits as a statement, we can just parse an expression.
        ASTNode *expr = parse_expression(parser);
    ...
    /* enable watchdog timeout <N>ms OR enable ota "name" [password "pw"] OR enable interrupts */
    if (parser_match(parser, TOK_ENABLE)) {
        lexer_next_token(parser->lexer);
        if (parser_match(parser, TOK_WATCHDOG)) {
            lexer_next_token(parser->lexer);
            if (parser_match(parser, TOK_TIMEOUT)) lexer_next_token(parser->lexer);
            Token ms_tok = parser->lexer->current_token;
            ms_tok.value = strdup(ms_tok.value);
            int ms = (int)atof(ms_tok.value);
            lexer_next_token(parser->lexer);
            if (parser_match(parser, TOK_MS_TOK)) lexer_next_token(parser->lexer);
    ...
    if (parser_match(parser, TOK_FEED)) {
        lexer_next_token(parser->lexer);
        if (parser_match(parser, TOK_WATCHDOG)) lexer_next_token(parser->lexer);
    ...
    if (parser_match(parser, TOK_DEFINE)) {
        ...
        if (parser_match(parser, TOK_TYPE_KW)) {
            ...
            while (!parser_match(parser, TOK_RBRACE) && !parser_match(parser, TOK_EOF)) {
                ...
                if (parser_match(parser, TOK_INT_KW))        { ftype = type_int();   lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_FLOAT_KW)) { ftype = type_float(); lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_BOOL_KW))  { ftype = type_bool();  lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_BYTE_KW))  { ftype = type_byte();  lexer_next_token(parser->lexer); }
                else if (parser_match(parser, TOK_VAR))      { lexer_next_token(parser->lexer); }
            ...
            fname_tok.value = strdup(fname_tok.value);
                ...
                if (!fname_tok.value || fname_tok.value[0] == '\0') {
                    error_report(parser->errors, ERROR_SYNTAX, fname_tok.line, fname_tok.column,
        ...
        if (parser_match(parser, TOK_DEVICE)) {
            ...
            dev_name.value = strdup(dev_name.value);
            parser_expect(parser, TOK_ID);
            if (parser_match_id(parser, "as")) lexer_next_token(parser->lexer);
            ProtocolType proto = PROTOCOL_UART;
            if (parser_match(parser, TOK_SERIAL))      { proto = PROTOCOL_UART; lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_I2C))    { proto = PROTOCOL_I2C;  lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_SPI))    { proto = PROTOCOL_SPI;  lexer_next_token(parser->lexer); }
            else if (parser_match(parser, TOK_ID) &&
                strcmp(parser->lexer->current_token.value,"uart")==0) {
                proto = PROTOCOL_UART; lexer_next_token(parser->lexer);
            }
            if (parser_match_id(parser, "at")) lexer_next_token(parser->lexer);
            ASTNode *addr = parse_expression(parser);
    ...
    if (parser_match(parser, TOK_START)) {
        lexer_next_token(parser->lexer);
        if (parser_match(parser, TOK_TASK)) lexer_next_token(parser->lexer);
        Token tname = parser->lexer->current_token;
            tname.value = strdup(tname.value);
            parser_expect(parser, TOK_ID);
    ...
    if (parser_match(parser, TOK_SHARED)) {
        lexer_next_token(parser->lexer);
        if (parser_match(parser, TOK_MAKE)) lexer_next_token(parser->lexer);
        Type *t = type_float();
        if (parser_match(parser, TOK_INT_KW))        { t = type_int();   lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_FLOAT_KW)) { t = type_float(); lexer_next_token(parser->lexer); }
        else if (parser_match(parser, TOK_BOOL_KW))  { t = type_bool();  lexer_next_token(parser->lexer); }
...
ASTNode* parser_parse(Parser *parser) {
    ...
    // Parse global top-level declarations (functions, variables, definitions, shared tasks)
    while (!parser_match(parser, TOK_PROGRAM) && !parser_match(parser, TOK_EOF)) {
        if (parser_match(parser, TOK_INCLUDE)) {
            ...
            if (parser_match(parser, TOK_STRING_LIT)) {
                ...
                if (!inc_file) {
                    error_report(parser->errors, ERROR_INTERNAL, parser->lexer->current_token.line, parser->lexer->current_token.column, "Could not open included file: %s", full_path);
                } else {
                    Parser *inc_parser = parser_create(inc_file, full_path, parser->errors);
                    ASTNode *inc_ast = parser_parse(inc_parser);
                    if (inc_ast && inc_ast->type == NODE_PROGRAM && inc_ast->data.program.main_block) {
        ...
        if (stmt) {
            ...
            error_report(parser->errors, ERROR_SYNTAX, parser->lexer->current_token.line, parser->lexer->current_token.column, "Unexpected token at global scope, skipping");
            lexer_next_token(parser->lexer);
        }
    }
...
static ASTNode* clone_target(ASTNode *node) {
    ...
    if (node->type == NODE_IDENTIFIER) {
        ...
        return ast_binary_op(node->data.binary_op.op, clone_target(node->data.binary_op.left), clone_target(node->data.binary_op.right));
    } else if (node->type == NODE_UNARY_OP) {
        return ast_unary_op(node->data.unary_op.op, clone_target(node->data.unary_op.operand));

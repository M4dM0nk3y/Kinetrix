...
static void codegen_expression(CodeGen *gen, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_I2C_READ:
            ...
            codegen_expression(gen, node->data.i2c.data);    // data field here holds the register address
    ...
    switch (node->type) {
        ...
        case NODE_ARRAY_DECL: {
            ...
            codegen_emit_line(gen, "%s %s[%d] = {0};\n",
                ctype, node->data.array_decl.name, node->data.array_decl.size);
            break;
        }
        ...
        case NODE_INTERRUPT_PIN: {
            ...
            codegen_emit_line(gen,
                "attachInterrupt(digitalPinToInterrupt(%d), _isr_pin%d, %s);\n",
        ...
        case NODE_INTERRUPT_TIMER:
            /* For Arduino/AVR we note the timer but can't add attachTimerInterrupt inline;
               a comment + MsTimer2 approach is emitted. */
            codegen_emit_line(gen, "/* timer interrupt every %d%s — use MsTimer2 or similar */\n",
        ...
        case NODE_WATCHDOG_ENABLE:
            codegen_emit_line(gen, "wdt_enable(WDTO_%dMS);\n",
                ...
                node->data.watchdog_enable.timeout_ms < 2000 ? 2000 : 4000);
            break;

        case NODE_OTA_ENABLE:
            codegen_emit_line(gen, "/* OTA not available on Arduino Uno (no WiFi) */\n");
...
static void codegen_assign_timer_ids(ASTNode *node, int *counter) {
    ...
    switch (node->type) {
        case NODE_PROGRAM:
            for (int i = 0; i < node->data.program.function_count; i++) codegen_assign_timer_ids(node->data.program.functions[i], counter);
            codegen_assign_timer_ids(node->data.program.main_block, counter);
            break;
        case NODE_BLOCK:
            for (int i = 0; i < node->data.block.statement_count; i++) codegen_assign_timer_ids(node->data.block.statements[i], counter);
            break;
        case NODE_FUNCTION_DEF: codegen_assign_timer_ids(node->data.function_def.body, counter); break;
        case NODE_TASK_DEF: codegen_assign_timer_ids(node->data.task_def.body, counter); break;
        case NODE_IF:
            ...
            break;
        case NODE_WHILE: codegen_assign_timer_ids(node->data.while_loop.body, counter); break;
        case NODE_FOR: codegen_assign_timer_ids(node->data.for_loop.body, counter); break;
        case NODE_REPEAT: codegen_assign_timer_ids(node->data.repeat_loop.body, counter); break;
        case NODE_INTERRUPT_PIN: codegen_assign_timer_ids(node->data.interrupt_pin.body, counter); break;
        case NODE_INTERRUPT_TIMER:
...
static void codegen_hoist_isrs(CodeGen *gen, ASTNode *node) {
    ...
    switch (node->type) {
        case NODE_PROGRAM:
            for (int i = 0; i < node->data.program.function_count; i++) codegen_hoist_isrs(gen, node->data.program.functions[i]);
            codegen_hoist_isrs(gen, node->data.program.main_block);
            break;
        case NODE_BLOCK:
            for (int i = 0; i < node->data.block.statement_count; i++) codegen_hoist_isrs(gen, node->data.block.statements[i]);
            break;
        case NODE_FUNCTION_DEF: codegen_hoist_isrs(gen, node->data.function_def.body); break;
        case NODE_TASK_DEF: codegen_hoist_isrs(gen, node->data.task_def.body); break;
        case NODE_IF:
            codegen_hoist_isrs(gen, node->data.if_stmt.then_block);
            codegen_hoist_isrs(gen, node->data.if_stmt.else_block);
            break;
        case NODE_WHILE: codegen_hoist_isrs(gen, node->data.while_loop.body); break;
        case NODE_FOR: codegen_hoist_isrs(gen, node->data.for_loop.body); break;
        case NODE_REPEAT: codegen_hoist_isrs(gen, node->data.repeat_loop.body); break;
        case NODE_INTERRUPT_PIN:
...
void codegen_generate_arduino(CodeGen *gen, ASTNode *program) {
    ...
    if (block && block->type == NODE_BLOCK) {
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ASTNode *stmt = block->data.block.statements[i];
            if (stmt && (stmt->type == NODE_VAR_DECL || stmt->type == NODE_ARRAY_DECL || stmt->type == NODE_BUFFER_DECL)) {
                codegen_statement(gen, stmt);
            }
        }
    }
            ...
            if (stmt && stmt->type == NODE_FUNCTION_DEF) {
                if (stmt->data.function_def.is_extern) {
                    ...
                    for (int j = 0; j < stmt->data.function_def.param_count; j++) {
                        ...
                        if (stmt->data.function_def.param_types && stmt->data.function_def.param_types[j])
                            pty = type_to_ctype(stmt->data.function_def.param_types[j]);
                        codegen_emit(gen, "%s %s", pty, stmt->data.function_def.param_names[j]);
                    }
                    codegen_emit(gen, ");\n\n");
                    continue;
                }
    ...
    if (block && block->type == NODE_BLOCK) {
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ...
            if (!stmt) continue;
            /* Skip hoisted stmts */
            if (stmt->type == NODE_FUNCTION_DEF) continue;
            if (stmt->type == NODE_STRUCT_DEF) continue;
            if (stmt->type == NODE_INTERRUPT_PIN) continue;
            if (stmt->type == NODE_TASK_DEF) continue;
            if (stmt->type == NODE_VAR_DECL || stmt->type == NODE_ARRAY_DECL || stmt->type == NODE_BUFFER_DECL) continue;
            codegen_statement(gen, stmt);
        }
    } else if (block) {

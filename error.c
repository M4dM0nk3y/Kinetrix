...
void error_report(ErrorList *list, ErrorType type, int line, int column,
                  const char *format, ...) {
  if (list == NULL)
    return;

  // Stop adding errors if we've hit the limit
  if (list->count >= list->max_errors)
    return;
...
void error_print_all(ErrorList *list, FILE *output) {
  ...
  const char *type_names[] = {"Lexical Error", "Syntax Error", "Semantic Error",

...
const char* type_to_string(Type *t) {
    ...
    switch (t->kind) {
        ...
        case TYPE_ARRAY:
            snprintf(buffer, sizeof(buffer), "%s[%d]", type_to_string(t->element_type), t->array_size);
            return buffer;
        case TYPE_BUFFER:
            snprintf(buffer, sizeof(buffer), "buffer<%s>[%d]", type_to_string(t->element_type), t->array_size);
            return buffer;
        case TYPE_STRUCT:
            snprintf(buffer, sizeof(buffer), "struct %s", t->struct_name ? t->struct_name : "?");
            return buffer;
...
ASTNode* ast_binary_op(Operator op, ASTNode *left, ASTNode *right) {
    ...
    if (op == OP_ADD && 
        ((left && left->value_type && left->value_type->kind == TYPE_STRING) || 
         (right && right->value_type && right->value_type->kind == TYPE_STRING))) {
        type_free(node->value_type);
        node->value_type = type_string();
    }
...
ASTNode* ast_serial_recv() {
    return ast_create(NODE_SERIAL_RECV);
}

/* --- I2C high-level --- */

ASTNode* ast_i2c_open() {
    return ast_create(NODE_I2C_OPEN);
}
...
ASTNode* ast_watchdog_feed() {
    return ast_create(NODE_WATCHDOG_FEED);
}

ASTNode* ast_ota_enable(const char *hostname, const char *password) {
    ASTNode *node = ast_create(NODE_OTA_ENABLE);
    node->data.ota_enable.hostname = hostname ? strdup(hostname) : strdup("kinetrix");
...
ASTNode* ast_enable_interrupts() {
    return ast_create(NODE_ENABLE_INTERRUPTS);
}
...
ASTNode* ast_radio_available() {
    return ast_create(NODE_RADIO_AVAILABLE);
}

ASTNode* ast_radio_read() {
    return ast_create(NODE_RADIO_READ);
}

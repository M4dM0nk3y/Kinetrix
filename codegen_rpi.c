...
static void rpi_expression(CodeGen *gen, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_ARRAY_ACCESS:
            ...
            break;
        case NODE_CAST:
            rpi_emit(gen, "%s(", strcmp(type_to_ctype(node->data.cast_op.target_type), "float") == 0 ? "float" : "int");
        ...
        case NODE_I2C_DEVICE_READ_ARRAY:
            break;
        case NODE_RADIO_AVAILABLE: rpi_emit(gen, "_radio_available()"); break;
        ...
        case NODE_MATH_FUNC: {
            ...
            switch(node->data.math_func.func) {
                case MATH_SIN: fn="math.sin"; break; case MATH_COS: fn="math.cos"; break;
                case MATH_TAN: fn="math.tan"; break; case MATH_SQRT: fn="math.sqrt"; break;
                case MATH_ASIN: fn="math.asin"; break; case MATH_ACOS: fn="math.acos"; break;
                case MATH_ATAN: fn="math.atan"; break; case MATH_ATAN2: fn="math.atan2"; break;
            }
            rpi_emit(gen, "%s(", fn);
            rpi_expression(gen, node->data.math_func.arg1);
            if(node->data.math_func.arg2) { rpi_emit(gen, ", "); rpi_expression(gen, node->data.math_func.arg2); }
            rpi_emit(gen, ")");
            break;
        }
...
static void rpi_statement(CodeGen *gen, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_SHARED_DECL:
            ...
            if (node->data.var_decl.initializer) rpi_expression(gen, node->data.var_decl.initializer);
            else rpi_emit(gen, "0");
            rpi_emit(gen, "\n");
            break;
        ...
        case NODE_FOR: {
            ...
            rpi_indent(gen);
            rpi_emit(gen, "for %s in range(_start_%d, _end_%d + (1 if _step_%d > 0 else -1), _step_%d):\n", 
        ...
        case NODE_PRINT:
            rpi_indent(gen); rpi_emit(gen, "print(");
            rpi_expression(gen, node->data.unary.child);
            rpi_emit(gen, ", end='')\n");
            break;
        ...
        case NODE_STRUCT_INSTANCE:
            rpi_indent(gen); rpi_emit(gen, "%s = {}\n", node->data.struct_instance.var_name);
            break;
        case NODE_INTERRUPT_PIN:
            rpi_indent(gen);
            rpi_emit(gen, "GPIO.add_event_detect(%d, GPIO.", node->data.interrupt_pin.pin_number);
            if(node->data.interrupt_pin.mode == INT_MODE_RISING) rpi_emit(gen, "RISING");
            else if(node->data.interrupt_pin.mode == INT_MODE_FALLING) rpi_emit(gen, "FALLING");
            else rpi_emit(gen, "BOTH");
        ...
        case NODE_INTERRUPT_TIMER:
            ...
            rpi_indent(gen); rpi_emit(gen, "def _timer_loop():\n");
            gen->indent_level++;
            rpi_indent(gen); rpi_emit(gen, "while True:\n");
            gen->indent_level++;
            rpi_indent(gen); rpi_emit(gen, "time.sleep(%g)\n", node->data.interrupt_timer.interval / (node->data.interrupt_timer.is_us ? 1000000.0 : 1000.0));
            rpi_indent(gen); rpi_emit(gen, "_isr_timer()\n");
            gen->indent_level--;
            gen->indent_level--;
            rpi_indent(gen); rpi_emit(gen, "threading.Thread(target=_timer_loop, daemon=True).start()\n");
            break;
        case NODE_DISABLE_INTERRUPTS: rpi_emit_line(gen, "pass # Disable IRQ not directly supported in user-space python"); break;
        case NODE_ENABLE_INTERRUPTS: rpi_emit_line(gen, "pass # Enable IRQ"); break;
        case NODE_I2C_OPEN: rpi_emit_line(gen, "_i2c = busio.I2C(board.SCL, board.SDA)"); break;
        case NODE_I2C_DEVICE_READ_ARRAY:
            rpi_indent(gen);
        ...
        case NODE_I2C_DEVICE_WRITE:
            /* Note: this is actually writing to an address not a register directly but let's emulate */
            rpi_indent(gen);
            ...
            rpi_emit(gen, ")\n");
            break;
        case NODE_SERIAL_OPEN: rpi_emit_line(gen, "import serial; _uart = serial.Serial('/dev/serial0', %d)", node->data.serial_open.baud_rate); break;
        case NODE_SERIAL_SEND:
            rpi_indent(gen); rpi_emit(gen, "if '_uart' in globals(): _uart.write(str(");
            rpi_expression(gen, node->data.serial_send.value); rpi_emit(gen, ").encode())\n"); break;
        case NODE_SPI_OPEN: rpi_emit_line(gen, "if '_spi' not in globals(): _spi = busio.SPI(clock=board.SCK, MISO=board.MISO, MOSI=board.MOSI)"); break;
        case NODE_DEVICE_DEF: rpi_indent(gen); rpi_emit(gen, "# Device %s configured\n", node->data.device_def.device_name); break;
        case NODE_DEVICE_WRITE: rpi_indent(gen); rpi_emit(gen, "# Write %s: ", node->data.device_write.device_name); rpi_expression(gen, node->data.device_write.value); rpi_emit(gen, "\n"); break;
        case NODE_TASK_DEF: break;
        case NODE_TASK_START:
            rpi_emit_line(gen, "import threading");
            rpi_emit_line(gen, "threading.Thread(target=_task_func_%s, daemon=True).start()", node->data.task_start.task_name);
            break;
        case NODE_TRY:
            rpi_emit_line(gen, "try:"); gen->indent_level++; rpi_statement(gen, node->data.try_stmt.try_block); gen->indent_level--;
            if (node->data.try_stmt.error_block) {
                rpi_emit_line(gen, "except Exception:"); gen->indent_level++; rpi_statement(gen, node->data.try_stmt.error_block); gen->indent_level--;
            }
            break;
        case NODE_ASSERT:
            rpi_indent(gen); rpi_emit(gen, "if not (");
            rpi_expression(gen, node->data.assert_stmt.condition);
            rpi_emit(gen, "):\n");
            gen->indent_level++;
            if(node->data.assert_stmt.action) rpi_statement(gen, node->data.assert_stmt.action);
            else { rpi_indent(gen); rpi_emit(gen, "raise AssertionError()\n"); }
            gen->indent_level--;
            break;
        ...
        case NODE_RADIO_SEND:
            ...
            rpi_emit(gen, ")\n");
            break;
        case NODE_WATCHDOG_ENABLE: rpi_emit_line(gen, "pass # WDT requires OS setup"); break;
        case NODE_WATCHDOG_FEED: rpi_emit_line(gen, "pass # WDT feed"); break;
        case NODE_OTA_ENABLE:
            ...
            rpi_emit_line(gen, "_ota_hostname = \"%s\"", node->data.ota_enable.hostname);
            rpi_emit_line(gen, "try:");
            rpi_emit_line(gen, "    subprocess.Popen(['avahi-publish-service', _ota_hostname, '_kinetrix._tcp', '5050', 'target=rpi'], stdout=subprocess.DEVNULL)");
            rpi_emit_line(gen, "    print(f\"OTA discoverable as {_ota_hostname} on {socket.gethostbyname(socket.gethostname())}\")");
            rpi_emit_line(gen, "except: print(\"Warning: avahi not available, OTA discovery disabled\")");
            break;
        case NODE_TONE:
            // RPi PWM starts by verifying object doesn't exist yet, if not it registers it globally, and changes its frequency using GPIO.ChangeFrequency. Wait, we can simplify this.
            rpi_indent(gen); rpi_emit(gen, "if '_pwm_%d' not in globals():\n", node->data.gpio.pin);
            gen->indent_level++;
            rpi_indent(gen); rpi_emit(gen, "GPIO.setup(%d, GPIO.OUT)\n", node->data.gpio.pin);
            rpi_indent(gen); rpi_emit(gen, "globals()['_pwm_%d'] = GPIO.PWM(%d, 1)\n", node->data.gpio.pin, node->data.gpio.pin);
            rpi_indent(gen); rpi_emit(gen, "globals()['_pwm_%d'].start(50)\n", node->data.gpio.pin);
            gen->indent_level--;
            rpi_indent(gen); rpi_emit(gen, "globals()['_pwm_%d'].ChangeFrequency(", node->data.gpio.value);
            rpi_expression(gen, node->data.gpio.value);
            rpi_emit(gen, ")\n");
            break;
        case NODE_NOTONE:
            rpi_indent(gen); rpi_emit(gen, "if '_pwm_%d' in globals():\n", node->data.gpio.pin);
            gen->indent_level++;
            rpi_indent(gen); rpi_emit(gen, "globals()['_pwm_%d'].stop()\n", node->data.gpio.pin);
            rpi_indent(gen); rpi_emit(gen, "del globals()['_pwm_%d']\n", node->data.gpio.pin);
            gen->indent_level--;
            break;
...
void codegen_generate_rpi(CodeGen *gen, ASTNode *program) {
    ...
    rpi_emit_line(gen, "        try: _i2c.writeto_then_readfrom(addr, bytes([reg]), res)");
    ...
    rpi_emit_line(gen, "    return 0  # Not strictly supported on RPi user-space");
    rpi_emit_line(gen, "");
    rpi_emit_line(gen, "def _radio_send(peer, data):");
    rpi_emit_line(gen, "    pass # Placeholder for RPi NRF24 or LoRa transmission");
    rpi_emit_line(gen, "");
    ...
    if (program->data.program.main_block &&
        ...
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ...
            if (stmt->type == NODE_VAR_DECL || stmt->type == NODE_ARRAY_DECL || stmt->type == NODE_BUFFER_DECL || stmt->type == NODE_SHARED_DECL) {
        ...
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ...
            if (stmt->type == NODE_INTERRUPT_PIN) {
                rpi_emit_line(gen, "def _isr_pin%d():", stmt->data.interrupt_pin.pin_number);
                gen->indent_level++;
        ...
        for (int i = 0; i < block->data.block.statement_count; i++) {
            ...
            if (stmt->type != NODE_FUNCTION_DEF && stmt->type != NODE_TASK_DEF && 
                ...
                stmt->type != NODE_INTERRUPT_PIN && stmt->type != NODE_INTERRUPT_TIMER) {
                rpi_statement(gen, stmt);
            }
        }
        gen->indent_level--;
